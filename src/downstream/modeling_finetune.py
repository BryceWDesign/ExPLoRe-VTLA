...
def register_model_safe(func):
    """Register model while suppressing overwrite warnings"""
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="Overwriting .* in registry")
        return register_model(func)
...
def beit_base_patch16_224(pretrained=False, **kwargs):
    # Remove timm-specific parameters for compatibility
    timm_params = ['pretrained_cfg', 'pretrained_cfg_overlay', 'cache_dir']
    for param in timm_params:
